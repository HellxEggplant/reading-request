...
MyMemory 只接收当前英文句子。选择 DeepSeek 时，只发送当前英文句子和文章标题，不发送完整 PDF、中文翻译、个人笔记、生词或整篇文章。API 密钥仅保留在当前浏览器标签页，关闭标签页后自动清除，不会进入 GitHub 或书库备份。MyMemory 单次请求存在 500 字节限制；AI 译文与机器译文都仅用于核对，不应视为唯一标准答案。
