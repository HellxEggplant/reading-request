...
一个面向个人英语阅读训练的浏览器应用。你可以上传英文期刊或杂志 PDF，框选其中的文章，把它转换成逐句精读材料；随后完成翻译、词汇查询、长难句标记和默写练习。
...
2. 点击右上角“上传 PDF”，选择一本英文期刊或杂志。
3. 在书库中点击“打开并框选文章”。
4. 翻到文章所在页面，开启“全屏框选”，按阅读顺序拖动框选正文区域。
5. 检查右侧识别文字，填写文章标题和 CEFR 难度，然后点击“生成逐句精读”。
...
1. 展开当前句子的“参考译文”。
2. 首次生成时，网站会询问是否允许把**当前英文句子**发送给 MyMemory 翻译服务。
3. 同意后生成英译中参考文本。
4. 参考译文可以编辑、保存，也可以重新生成。

网站不会在此过程中发送完整 PDF、个人笔记或其他文章。MyMemory 单次请求存在 500 字节限制，过长句子需要手动填写或拆分。机器译文仅用于核对，不应视为唯一标准答案。
...
当前版本是个人阅读训练工具，不包含教师、班级、公开排行榜、积分或卡牌系统。

## 数据保存、备份与换电脑

### 数据保存在哪里

- PDF、文章、翻译、笔记、词汇和完成状态保存在浏览器的 IndexedDB 中。
- 引导提示和在线翻译授权等轻量设置保存在 localStorage 中。
...
- Datamuse 词典和 MyMemory 参考翻译是在线服务，断网时不可用。
