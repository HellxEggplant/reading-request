...
- 单词和音标；
- 浏览器语音合成发音；
- DeepSeek AI 结合文章标题、当前句及前后各最多两句生成的中文语境释义、词性说明和语境搭配；
- ECDICT 本地中英释义；
- Datamuse 在线英英释义与相关词；
...
> AI 语境释义复用“AI 与参考译文设置”中的 DeepSeek API 密钥。首次使用时，网站会明确询问是否发送目标单词、文章标题、所在句及前后各最多两句；不会发送 PDF、整篇文章、中文翻译、生词表或个人笔记。ECDICT 词库按常用词拆分为静态数据文件；Datamuse 查询需要联网。
...
1. 展开当前句子的“参考译文”。
2. 默认使用 MyMemory；也可以点击参考译文框下方的齿轮，在“AI 与参考译文设置”中切换到 DeepSeek API，并填写自己的 DeepSeek API 密钥。密钥默认隐藏，点击输入框右侧的“显示/隐藏”按钮可在当前页面临时查看。
3. 同意后生成英译中参考文本。
