...
| 逐句精读 | 按句学习文章 | 前后翻句、翻译、笔记、校正文句、生词和长难句标记、沉浸模式与番茄专注 |
| AI 双语词典 | 理解并积累词汇 | DeepSeek 中文/英文释义、语境义、音标、搭配、例句、发音和个人笔记 |
| 翻译学习 | 核对并改进自己的翻译 | DeepSeek 参考译文、自动校验、评分、问题反馈和整合译文 |
| AI 精灵 | 随时解决精读问题 | 围绕当前句连续询问词义、语法、结构与翻译 |
...
- 专注阶段默认 25 分钟，休息阶段为 5 分钟；面板中可直接切换“专注 25:00”和“休息 05:00”。
...
- 默认开启“结束后自动开始下一阶段”：专注完成会自动开始 5 分钟休息，休息结束会自动开始下一轮专注；可在面板中关闭自动接续。

### 沉浸模式

点击精读页顶部的“沉浸模式”，网站会尝试进入浏览器全屏，并以一页一句的方式展示文章：

- 使用底部按钮或键盘 `←`、`→` 切换句子；顶部进度条显示整篇进度。
- 英文单词仍可点击查询；中文理解可以直接填写，并与普通精读卡片同步。
- 沉浸模式中仍会显示可拖动的番茄时钟，可直接开始 25 分钟专注或 5 分钟休息。
- 点击“退出”或按 `Esc` 离开沉浸模式，当前翻译会自动保存。
...
- 单词和音标；
- 浏览器语音合成发音；
- DeepSeek 生成的中文释义和英文释义；
- 音标、词性、本文语境义、相关搭配和双语例句；
- 可编辑、保存的“我的中文笔记”。

已经标记的词会显示在句子下方的生词栏。点击词旁的“×”可以取消标记；点击词本身可以再次打开详情。查询结果和个人笔记会跟随文章保存在本地。

> 双语词典复用“DeepSeek AI 设置”中的 API 密钥。首次使用时，网站会明确询问是否发送目标单词、文章标题、所在句及前后各最多两句；不会发送 PDF、整篇文章、中文翻译、生词表或个人笔记。配置 DeepSeek 后不会调用 ECDICT、Datamuse 或其他词典服务。
...
1. 展开当前句子的“参考译文”。
2. 点击参考译文框下方的齿轮，在“DeepSeek AI 设置”中填写自己的 API 密钥。密钥默认隐藏，点击输入框右侧的“显示/隐藏”按钮可在当前页面临时查看。
3. 同意后生成英译中参考文本。
4. 参考译文可以编辑、保存，也可以重新生成。

参考译文统一使用 DeepSeek。网站会把文章标题、当前目标句及其前后各最多两句作为上下文发送给 DeepSeek；模型只翻译目标句，相邻句仅用于判断词义、指代、时态和语气。不会发送完整 PDF、个人笔记、生词或整篇文章。API 密钥仅保留在当前浏览器标签页，关闭标签页后自动清除，不会进入 GitHub 或书库备份。

### AI 校验与整合译文

写完自己的中文翻译并生成参考译文后，点击“校验我的翻译”。结果显示在“语法或阅读笔记”下方的“AI 阅读建议”中，可以折叠，包括优点、误译或遗漏和整合译文；个人笔记独立保留。点击“采用这版译文”才会把整合结果填入自己的翻译框。

### AI 精灵

精读页面提供“AI 精灵”。点击打开聊天，按住入口可自由拖动，位置会在当前浏览器记住；窗口会根据位置向上或向下展开。它会读取当前句、相邻句、当前译文和参考译文，可以连续询问词义、语法、结构、指代和翻译差异。切换文章时对话会清空。
...
- 引导提示、翻译服务和模型选择等轻量设置保存在 localStorage 中；DeepSeek API 密钥只临时保存在 sessionStorage 中。
...
- [DeepSeek API](https://api-docs.deepseek.com/)：生成双语词典、上下文参考译文、译文校验和 AI 精灵回答。
