...
| 逐句精读 | 按句学习文章 | 前后翻句、翻译、笔记、校正文句、生词和长难句标记、沉浸模式、番茄专注与轻音乐 |
...
- 点击右上角“轻音乐”打开声音空间设置。内置“清溪”轻音乐采用舒缓五声音阶，无需联网，可调整音量，并可设置每次进入沉浸模式时自动播放。
- 沉浸模式中仍会显示可拖动的番茄时钟，可直接开始 25 分钟专注或 5 分钟休息。
- 选择“自定义地址”后，可以填写浏览器能够直接播放的 MP3、M4A、OGG、WAV 或 MP4 文件地址。MP4 会只输出声音，不显示视频画面。
- YouTube、哔哩哔哩等视频网站的普通页面地址通常不能当作媒体直链使用，这取决于来源网站的权限和跨域限制。
- 点击“退出”或按 `Esc` 离开沉浸模式，当前翻译会自动保存，音乐同时停止。
...
1. 展开当前句子的“参考译文”。
2. 默认使用 MyMemory；也可以点击参考译文框下方的齿轮，在“翻译模型设置”中切换到 DeepSeek API，并填写自己的 DeepSeek API 密钥。密钥默认隐藏，点击输入框右侧的“显示/隐藏”按钮可在当前页面临时查看。
3. 同意后生成英译中参考文本。
4. 参考译文可以编辑、保存，也可以重新生成。

MyMemory 只接收当前英文句子。选择 DeepSeek 时，会把文章标题、当前目标句及其前后各最多两句作为上下文发送给 DeepSeek；模型只翻译目标句，相邻句仅用于判断词义、指代、时态和语气。不会发送完整 PDF、中文翻译、个人笔记、生词或整篇文章。API 密钥仅保留在当前浏览器标签页，关闭标签页后自动清除，不会进入 GitHub 或书库备份。MyMemory 单次请求存在 500 字节限制；AI 译文与机器译文都仅用于核对，不应视为唯一标准答案。
...
- Datamuse 词典、MyMemory 和 DeepSeek 参考翻译都是在线服务，断网时不可用。
